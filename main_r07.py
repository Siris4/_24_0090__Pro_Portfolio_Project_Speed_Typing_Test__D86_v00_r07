...
def calculate_speed():
    ...
    # Unbind both the spacebar and Enter/Return keys to prevent interaction after the test ends
    typing_input.unbind("<space>")
    typing_input.unbind("<Return>")

    # Show the result without accidental keypresses closing it
    messagebox.showinfo("Result", f"Typing Speed: {wpm:.2f} WPM")

    # Enable the start button after the test is done
...
def start_test():
    ...
    # Bind both Spacebar and Enter/Return to submit the word
    typing_input.bind("<space>", check_word)
    typing_input.bind("<Return>", check_word)
